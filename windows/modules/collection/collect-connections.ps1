...
# Get Connections
Get-NetTCPConnection | select localaddress,localport,remoteadress,remoteport,state,owningprocess |
foreach { 
    
    # Get connections variables
    $ConnLocalAddress = $_.LocalAddress
    $ConnLocalPort = $_.LocalPort
    $ConnRemoteAddress = $_.RemoteAddress
    $ConnRemotePort = $_.RemotePort
    $ConnState = $_.State
    $ConnProcess = $_.OwningProcess

    # Check process
	$FinalOutput  = Get-WMIObject Win32_Process | Select name,ProcessId,description, commandline,creationdate | where processid -like "$ConnProcess" |
    foreach{

        # Check process binary 
       # Parse exe path
        if ($_.pathname -like "*`"*"){
            $ExePath = $_.commandline -split("`"")
        ...
        if(Test-Path $TargetPath){
            ...
            # Grab PE info
            $PEConfig = Get-PESecurity -File $TargetPath
            $ARCH = $PEConfig.ARCH
            $ASLR = $PEConfig.ASLR
	...
	$Object | add-member DataSource2 	  "Get-NetTCPConnection and Get-WMIObject Win32_Process"	
        ...
        $Object | add-member RemoteAddress        $ConnRemoteAddress                    
        $Object | add-member RemotePort           $ConnRemotePort
        $Object | add-member State                $ConnState
