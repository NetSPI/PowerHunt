...
#          and is used to collect information from windows services.
...
$FinalOutput = Get-WmiObject -Class win32_service | Select Name,ServiceName,Description,PathName,ServiceType,StartMode,Status,InstallDate,StartName  | 
foreach {

    # Parse exe path
    if ($_.pathname -like "*`"*"){
        $ExePath = $_.pathname -split("`"")
        $TargetPath = "`"" + $ExePath[1] + "`""
        #$TargetPath
    }else{
        $ExePath = $_.pathname -split(".exe")
    ...
    $Object | add-member StartMode            $_.StartMode
    $Object | add-member Status               $_.Status
    $Object | add-member InstallDate          $_.InstallDate
