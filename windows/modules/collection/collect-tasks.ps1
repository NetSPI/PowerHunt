...
#  Load Get-PESecurity
# ///////////////////////////////////////
# Source: https://github.com/NetSPI/PESecurity/blob/master/Get-PESecurity.psm1
...
# ///////////////////////////////////////
$taskPath = "\*\"
$FinalOutput = Get-ScheduledTask -TaskPath $taskPath |
ForEach-Object { 

    $TargetPath = $_.Actions.execute

    # If no drive letter in taretpath then assume system32.
    if($TargetPath -notlike "*`:*" -and $TargetPath -notlike "*%*" -and $TargetPath -notlike ''){
        if(test-path ("c:\windows\system32\" + $TargetPath)){
            $TargetPath = "c:\windows\system32\" + $TargetPath
        }else{
            $TargetPath = "c:\windows\"+ $TargetPath
        }
    }

    # If %windir% then swap with ps
    if($TargetPath -like "*%windir%*"){
        $TargetPath = $TargetPath -Replace('%windir%',$env:windir)
    }

    # If %SystemRoot% then swap with ps
    if($TargetPath -like "*%SystemRoot%*"){
        $TargetPath = $TargetPath -Replace('%SystemRoot%',$env:SystemRoot)
    }
    
    # Grab info from PE
	if($TargetPath -like "*.exe"){
	    $PEConfig = Get-PESecurity -File "$TargetPath" -ErrorAction SilentlyContinue
	    $ARCH = $PEConfig.ARCH
	    $ASLR = $PEConfig.ASLR
	    $Authenticode = $PEConfig.Authenticode
	    $ControlFlowGuard = $PEConfig.ControlFlowGuard
	    $DEP = $PEConfig.DEP
    ...
    $Triggers = $_.Triggers
    $Triggers = $_.Triggers
    $TriggerDelay = $Triggers.Delay
    $TriggerEnabled = $Triggers.Enabled
    $TriggerEndBoundary = $Triggers.EndBoundary
    $TriggerExecutionTimeLimit = $Triggers.ExecutionTimeLimit
    $TriggerId = $Triggers.Id
    $TriggerPSComputerName = $Triggers.PSComputerName
    $TriggerRepetition = $Triggers.Repetition
    $TriggerStartBoundary = $Triggers.StartBoundary
    $TriggerStateChange = $Triggers.StateChange
    $TriggerSubscription = $Triggers.Subscription
    $TriggerUserId = $Triggers.UserId
    $TriggerValueQueries = $Triggers.ValueQueries
    $TriggerStateChange = $Triggers.StateChange
            
    # Create new object to return
    $Object = New-Object PSObject
	$Object | add-member DataSource1 "task"
	$Object | add-member DataSource2 "Get-ScheduledTask"

    # Core task info
	$Object | add-member TaskName	          $_.TaskName
	$Object | add-member LastResult           $(($_ | Get-ScheduledTaskInfo).LastTaskResult)
	$Object | add-member NextRun	          $(($_ | Get-ScheduledTaskInfo).NextRunTime)
	$Object | add-member Status	              $_.State
	$Object | add-member Author	              $_.Author
	$Object | add-member UserId	              $_.principal.UserId
	$Object | add-member Arguments	          $_.Actions.Arguments 
	$Object | add-member PathName	          $_.TaskPath
    $Object | add-member AllowDemandStart     $_.Settings.AllowDemandStart 
    $Object | add-member AllowHardTerminate   $_.Settings.AllowHardTerminate
    $Object | add-member Enabled              $_.Settings.Enabled
    $Object | add-member Hidden               $_.Settings.Hidden
    $Object | add-member Priority             $_.Settings.Priority
    $Object | add-member RunOnlyIfIdle        $_.Settings.RunOnlyIfIdle
    $Object | add-member RunOnlyIfNetworkAvailable  $_.Settings.RunOnlyIfNetworkAvailable
    $Object | add-member StartWhenAvailable         $_.Settings.StartWhenAvailable
    $Object | add-member WakeToRun            $_.Settings.WakeToRun

    # Trigger info
    $Object | add-member TriggerDelay $TriggerDelay
    $Object | add-member TriggerEnabled $TriggerEnabled
    $Object | add-member TriggerEndBoundary $TriggerEndBoundary
    $Object | add-member TriggerExecutionTimeLimit $TriggerExecutionTimeLimit
    $Object | add-member TriggerId $TriggerId
    $Object | add-member TriggerPSComputerName $TriggerPSComputerName
    $Object | add-member TriggerRepetition $TriggerRepetition
    $Object | add-member TriggerStartBoundary $TriggerStartBoundary
    $Object | add-member TriggerStateChange $TriggerStateChange
    $Object | add-member TriggerSubscription $TriggerSubscription
    $Object | add-member TriggerUserId $TriggerUserId
    $Object | add-member TriggerValueQueries $TriggerValueQueries
	...
	$Object | add-member FileLastAccessTime   $FileLastACcess
	$Object | add-member ARCH	              $ARCH
	$Object | add-member ASLR	              $ASLR 
	$Object | add-member Authenticode         $Authenticode
	$Object | add-member ControlFlowGuard     $ControlFlowGuard
	$Object | add-member DEP	              $DEP
	$Object | add-member DotNET	              $DotNET
