...
#          and is used to collect information from registry run keys.
...
function Invoke-HuntRegistryAutorun {
    ...
    $TableResults.Columns.Add('FileLastAccessTime') | Out-Null
    $TableResults.Columns.Add('ARCH') | Out-Null
    $TableResults.Columns.Add('ASLR') | Out-Null
    $TableResults.Columns.Add('Authenticode') | Out-Null
    $TableResults.Columns.Add('ControlFlowGuard') | Out-Null
    $TableResults.Columns.Add('DEP') | Out-Null
    $TableResults.Columns.Add('DotNET') | Out-Null
    $TableResults.Columns.Add('HighentropyVA') | Out-Null
    $TableResults.Columns.Add('SafeSEH') | Out-Null
    $TableResults.Columns.Add('StrongNaming') | Out-Null
    ...
    ForEach-Object {
        ...
        ForEach-Object {
            ...
            # Parse exe path
            if ($ResultValue -like "*`"*"){
                $ExePath = $ResultValue -split("`"")
                $TargetPath = "`"" + $ExePath[1] + "`""
            }else{
                $ExePath = $ResultValue -split(".exe")
            ...
            # Add record to table
            $TableResults.Rows.Add("autorun registry keys",
								   "run keys",
								   $RegPsPath,
                                   $RegPsParentPath,
                                   $RegPsChildName,
...
# Run function
$FinalOutput = Invoke-HuntRegistryAutorun
